...
const HomePage = () => {
  ...
  return (
    <div
      className="font-[-apple-system,BlinkMacSystemFont,'Segoe UI'] flex flex-col min-h-screen w-full"
      style={{
    ...
    >
      ...
      {/* 主体内容 */}
      <main className="grid md:grid-cols-2 gap-8 p-4 sm:p-8 md:p-16 flex-grow">
        {/* 左侧内容 */}
        ...
        <section className="flex flex-col justify-center items-center">
          {isCameraOn ? (
            <div className="flex flex-col items-center w-full">
              <video ref={videoRef} autoPlay playsInline className="w-full max-w-lg rounded-2xl mb-4" />
              <button
                onClick={handleStopCamera}
                className="flex justify-center items-center gap-x-2 w-full max-w-xs py-3 px-8 rounded-2xl font-medium"
                style={{ backgroundColor: "rgba(239, 68, 68, 1)", color: "white" }}
              >
            ...
            <>
              <img
                src="https://static.paraflowcontent.com/public/resource/image/bd36ae9e-94ac-4ebf-82d2-9b6ad3084eb3.jpeg"
                alt="A happy person hugging a dog"
                className="w-full max-w-lg h-auto mb-6 rounded-2xl"
              />
              <div className="flex flex-col items-center mt-4">
                <p className="text-center mb-6 text-sm italic" style={{ color: "rgba(130, 130, 130, 1)" }}>
                  您的完美伴侣正在等待您！
                </p>
                <button
                  onClick={handleStartCamera}
                  className="flex justify-center items-center gap-x-2 w-full max-w-xs py-3 px-8 rounded-2xl font-medium"
                  style={{ backgroundColor: "rgba(255, 217, 102, 1)", color: "rgba(93, 64, 55, 1)" }}
                >
                  <i className="fas fa-paw"></i>
                  <span>开始匹配</span>
                </button>
                <p className="text-center mt-3 text-xs" style={{ color: "rgba(130, 130, 130, 1)" }}>
                  继续操作即表示您同意我们的服务条款
                </p>
              </div>
            </>
