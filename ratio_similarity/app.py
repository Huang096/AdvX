...
DOG_RATIO_DIR = "/Users/huangzheheng/Desktop/AdvX/ratio_similarity/data/dogRatios"
DOG_IMAGE_DIR = "/Users/huangzheheng/Desktop/DogFLW/test/images"
...
# 每个 facial ratio 的权重（可自行调节）
RATIO_WEIGHTS = {
    "eye_distance_ratio": 2.2,             # 眼距
    "mouth_eye_ratio": 1.5,                # 嘴宽和眼间距
    "nose_mouth_vertical_ratio": 1.0,      # 鼻子到嘴的垂直比例
    "chin_nose_to_upperface_ratio": 5    # 脸长与上脸高度（脸型）
}
...
def ratio_distance(dog_ratios, human_ratios):
    ...
    for dog_k, human_k in FIELD_MAP.items():
        if dog_k in dog_ratios and human_k in human_ratios:
            weight = RATIO_WEIGHTS.get(dog_k, 1.0)
...
def ratio_match():
    ...
    with TemporaryDirectory() as td:
        ...
        for path in dog_files:
            ...
            image_name = os.path.basename(path).replace("_ratios.json", ".png")
            source_image_path = os.path.join(DOG_IMAGE_DIR, image_name)
            static_image_path = os.path.join(STATIC_IMG_DIR, image_name)

            if os.path.exists(source_image_path):
                shutil.copy(source_image_path, static_image_path)
                all_results.append({
